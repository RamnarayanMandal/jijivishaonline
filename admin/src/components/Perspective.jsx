...
const Perspective = () => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(
        `${URI}api/perspective/update-Perspective/${selectedBlog._id}`,
        data
      );
      // Update UI after successful update
      const updatedBlogs = blogs.map((blog) =>
        blog._id === selectedBlog._id
          ? { ...blog, ...formData, image: blog.image }
          : blog
      );
          ...
          <ContentWrapper>
            <TextWrapper>
              <Typography
                variant="h5"
                component="h2"
                sx={{ fontWeight: "bold", color: "black" }} // Text color black
              >
                {blog.title}
              </Typography>
              <Typography variant="body1" sx={{ marginBottom: "8px", color: "black" }}>
                {blog.description}
              </Typography>
              <Typography variant="body2" sx={{ marginBottom: "8px", color: "black" }}>
                {blog.description2}
              </Typography>
              <Typography variant="body2" sx={{ marginBottom: "8px", color: "black" }}>
                {blog.description3}
      ...
      <Modal open={open} onClose={handleClose}>
        <StyledModal>
          <Typography
            variant="h6"
            component="h2"
            sx={{ marginBottom: "16px", color: "black" }} // Text color black
          >
            Edit Blog
          </Typography>
          <TextField
            ...
            sx={{ color: "black" }} // Input text color black
          />
          <TextField
            label="Description"
            name="description"
          ...
          <Button
            ...
            component="label"
            fullWidth
            sx={{ marginTop: "16px", marginBottom: "16px", color: "black" }}
          >
            Upload Image
