...
import { FaMoneyBillWave, FaUniversity, FaCreditCard, FaCcVisa, FaGoogleWallet } from 'react-icons/fa';
...
const PaymentHomepage = () => {
    ...
    const calculateTotal = () => {
        return bag.data.reduce((total, item) => total + (item.price * item.quantity), 0);
    };
        ...
        <>
            ...
            <div className="payment-bill-container p-6 max-w-5xl mx-auto bg-white shadow-md rounded-md w-full flex flex-col md:flex-row">
                <div className="payment-methods w-full md:w-1/2 p-6 border-r border-gray-300">
                    ...
                    <div onClick={() => handlePaymentChange('Cash on Delivery')} className={`mb-4 flex items-center cursor-pointer ${getTextColor('Cash on Delivery')}`}>
                        <FaMoneyBillWave className="text-green-500 text-2xl mr-2" />
                        Cash on Delivery
                    </div>
                    <div onClick={() => handlePaymentChange('Net Banking')} className={`mb-4 flex items-center cursor-pointer ${getTextColor('Net Banking')}`}>
                        <FaUniversity className="text-blue-500 text-2xl mr-2" />
                        Net Banking
                    </div>
                    <div onClick={() => handlePaymentChange('Card Payment')} className={`mb-4 flex items-center cursor-pointer ${getTextColor('Card Payment')}`}>
                        <FaCreditCard className="text-purple-500 text-2xl mr-2" />
                        Card Payment
                    </div>
                    <div onClick={() => handlePaymentChange('Credit Card')} className={`mb-4 flex items-center cursor-pointer ${getTextColor('Credit Card')}`}>
                        <FaCcVisa className="text-blue-700 text-2xl mr-2" />
                        Credit Card
                    </div>
                    <div onClick={() => handlePaymentChange('UPI')} className={`mb-4 flex items-center cursor-pointer ${getTextColor('UPI')}`}>
